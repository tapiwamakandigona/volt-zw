...
/** "Continue with Google" button — works on web (redirect) and native (in-app). */
export default function GoogleButton({ label = "Continue with Google" }: { label?: string }) {
  ...
  async function onClick() {
    ...
    try {
      ...
      // Web path redirects away; native path resolves here on success.
      if (isNative()) router.replace("/app");
    } catch (e) {
